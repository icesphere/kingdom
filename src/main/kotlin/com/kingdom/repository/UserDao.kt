...
class UserDao {
    ...
    private List<User> getUsingDeckFrequencies() {
        ...
        SQLQuery sqlQuery = session.createSQLQuery("select u.* from users u where u.active = 1 and u.admin = 0 and (base_weight != 3 or intrigue_weight != 3 or seaside_weight != 3 or prosperity_weight != 3 or cornucopia_weight != 3 or hinterlands_weight != 3 or promo_weight != 3) order by u.last_login desc");
        sqlQuery.addEntity(User.class);
        List<User> users = sqlQuery.list();

...
abstract class BotPlayer(user: User, game: Game) : Player(user, game) {
    ...
    private val cardsToPlay: List<Card>
        get() {
            val actionCards = hand.filter { it.isAction }.filter { getPlayCardScore(it) >= 0 && canPlayCardFromHand(it) }
            val treasureCards = hand.filter { it.isTreasure }.filter { getPlayCardScore(it) >= 0 && canPlayCardFromHand(it) }

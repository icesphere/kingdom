<div class="cardName" style="<#if clickType=='supply' && gameStatus == "InProgress" && supply(card.pileName) == 0><#else><#if card.backgroundColor.image>background-image: url(images/${card.backgroundColorMobileColor});background-repeat: repeat-x;<#else>background-color:${card.backgroundColorColor};</#if></#if>">
    ...
    <#if card.omen>
        <div style="position:absolute; right:3px; bottom:12px; background-color:#FFF4BE; border:1px solid #C98A00; border-radius:4px; padding:0 2px; font-size:8px; font-weight:bold; line-height:10px;">+1 Sun</div>
    </#if>
    ...
    <#if (clickType=='supply' || clickType == 'landmark') && victoryPointsOnSupplyPile(card.name)?? && victoryPointsOnSupplyPile(card.name) != 0><div style="position: absolute; bottom: 3px; left: 17px; z-index: 5; color: #00BB00; font-size: 8px; font-weight: bold; padding: 0 2px 0 2px;background-color: #ffffff77;">${victoryPointsOnSupplyPile(card.name)} VP on pile</div></#if>
    <#if clickType=='supply' && debtOnSupplyPile(card.name)?? && debtOnSupplyPile(card.name) != 0><div style="position: absolute; bottom: 10px; left: 14px; z-index: 5; color: #BB0000; font-size: 8px; font-weight: bold; padding: 0 2px 0 2px;background-color: #ffffff77;">${debtOnSupplyPile(card.name)} debt on pile</div></#if>
